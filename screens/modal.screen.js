import {View, Text, FlatList, StyleSheet, Dimensions, TouchableWithoutFeedback,
Modal, Keyboard} from 'react-native';
import Input from '../components/Input.component';
import SwitchInput from '../components/switchInput.component';
...
const ModalScreen = ({navigation}) => {
    ...
    <Modal visible={message.length > 0} animationType={"slide"}
    transparent={true} >
        <View style={styles.modalContainer}>
            <Text style={styles.modalText}>{message}</Text>
        </View>
    </Modal>
    <Input value={paletteName} onChange={(e) => setPaletteName(e.nativeEvent.text)} label="Palette Name:"/>
    <View style={
        styles.listHeight
    }>
    <FlatList
    ...
    renderItem = {
        ({item}) => {
            return <SwitchInput color={item} selectedColors={selectedColor} colorHandler={setSelectedColor} />
        }
    }
    ...
    <TouchableWithoutFeedback onPress={
        () => {
          if (paletteName.length === 0 || selectedColor.length < 5) {
            raiseMessage('Please enter a palette name and select at least five color');
          }
          else {
            navigation.navigate('Home', {paletteName, colors: selectedColor});
          }
        }
    }>
